...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
    ...
    // Make a copy of the query and convert ::tolower to compare regular expressions
    string copy(sql);
    transform(copy.begin(), copy.end(), copy.begin(), ::tolower);

    // Checking regex to find OR something=something type of SQL Injection attack
    regex something("or\\s([0-9]+)=\\1|(['\"][\\w]+['\"]+)=\\2", regex::icase);
    smatch posID;

    // Compare query to known patterns for a SQL Injection attack and output alert and exit
    if (regex_search(copy, posID, something) && posID.size() > 1) {
        cout << "WARNING: SQL INJECTION ATTACK DETECTED!" << endl;
