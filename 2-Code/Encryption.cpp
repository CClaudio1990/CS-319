...
std::string read_file(const std::string& filename)
{
    ...
    std::string file_text = std::string((std::istreambuf_iterator<char>(my_fs)), std::istreambuf_iterator<char>());
...
void save_data_file(const std::string& filename, const std::string& student_name, const std::string& key, const std::string& data)
{
    ...
    std::time_t time_stamp = time(0);
    localtime_s(&ltm, &time_stamp); std::strftime(tsBuffer, sizeof(tsBuffer), "%Y-%m-%d", &ltm);

    my_ofstream << student_name << std::endl;
    my_ofstream << tsBuffer << std::endl;
    my_ofstream << key << std::endl;
    my_ofstream << data << std::endl;
    my_ofstream.close();
}
